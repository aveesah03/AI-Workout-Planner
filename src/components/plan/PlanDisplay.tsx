...
function ExerciseRow({
  ...
  return (
    <tr className="border-b border-[var(--gym-color-border)] last:border-0">
      <td className="py-3 pr-4">
        <div className="flex items-start gap-3">
          <span className="text-xs text-[var(--gym-color-muted)] w-5">
            {index + 1}.
          </span>
          <div>
            <p className="font-medium">{exercise.name}</p>
            {exercise.notes && (
              <p className="text-xs text-[var(--gym-color-muted)] mt-0.5 flex items-center gap-1">
      ...
      <td className="py-3 px-4 text-center whitespace-nowrap">
        <span className="text-[var(--gym-color-accent)] font-medium">
          {exercise.sets}
        </span>
        <span className="text-[var(--gym-color-muted)]"> x </span>
        <span>{exercise.reps}</span>
      </td>

      <td className="py-3 px-4 text-center">
        <span className="text-[var(--gym-color-muted)]">{exercise.rest}</span>
      </td>
      <td className="py-3 px-4 text-center">
        <span
    ...
    <Card variant="bordered" className="overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-[var(--gym-color-accent)]">{schedule.focus}</p>
        </div>
        <div className="flex items-center gap-2 text-sm text-[var(--gym-color-muted)]">
      ...
      <div className="overflow-x-auto -mx-6 px-6">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[var(--gym-color-muted)] text-xs uppercase tracking-wider">
